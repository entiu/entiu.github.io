...
function giveDefinition() {
    ...
    xhr.onload = function() {
        if (this.status !== 200) {
            definition_ui.innerHTML += `<h4>Whoopsie, no definitions found for ${word}... Maybe <a href="https://www.google.com/search?q=${word}+meaning" target="_blank">ask Google</a>?</h4>`;
            return;
        };
        ...
        definition_ui.innerHTML += `<h2>${word_local} <span class="phonetic">${typeof phonetic !== 'undefined' ? '[' + phonetic + ']' : ''}</span></h2>`
