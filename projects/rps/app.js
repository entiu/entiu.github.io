...
var getWeapon = fighter => fighter.dataset.weapon;

var getDistance = (f1, f2) => Math.sqrt((f1.dataset.left - f2.dataset.left) ** 2 + (f1.dataset.top - f2.dataset.top) ** 2);

var getTarget = fighter => {
  var id = '';
  var distance = Math.sqrt(boundX ** 2 + boundY ** 2); // start with the farthest possible distance on the bounded board: the diagonal
  fighters.forEach(f => {
...
var move = (fighter, target, direction) => {
  ...
  var moveX = (deltaX * speed / dist + Math.random() * randomMultiplier + parseFloat(velocity[getWeapon(fighter)])) * (fighterRect.left < targetRect.left ? 1 : (-1)) * (direction === 'closer' ? 1 : (-1));
  var moveY = (deltaY * speed / dist + Math.random() * randomMultiplier + parseFloat(velocity[getWeapon(fighter)])) * (fighterRect.top < targetRect.top ? 1 : (-1)) * (direction === 'closer' ? 1 : (-1));
...
//   var newX = (parseFloat(fighter.dataset.left) + (deltaX * speed / dist /* + velocity[getWeapon(fighter)] / 20 */) * (fighterRect.left < targetRect.left ? 1 : (-1))).toFixed(2);
//   var newY = (parseFloat(fighter.dataset.top) + (deltaY * speed / dist /* + velocity[getWeapon(fighter)] / 20 */) * (fighterRect.top < targetRect.top ? 1 : (-1))).toFixed(2);
...
// var moveFarther = (fighter, target) => {
//   var fighterRect = fighter.getBoundingClientRect();
//   var targetRect = target.getBoundingClientRect();
...
//     fighter.style.left = `${newX}px`;
//   } /* else if (newX < 0) {
//     fighter.dataset.left = newX + deltaX * speed / dist;
//     fighter.style.left = `${newX + deltaX * speed / dist}px`;
//   } else {
//     fighter.dataset.left = newX - deltaX * speed / dist;
//     fighter.style.left = `${newX - deltaX * speed / dist}px`;
//   } */
...
//     fighter.style.top = `${newY}px`;
//   } /* else if (newY < 0) {
//     fighter.dataset.top = newY + deltaY * speed / dist;
//     fighter.style.top = `${newY + deltaY * speed / dist}px`;
//   } else {
//     fighter.dataset.top = newY - deltaY * speed / dist;
//     fighter.style.top = `${newY - deltaY * speed / dist}px`;
//   } */
...
var updateVelocity = () => {
  for (weapon in velocity) {
    velocity[weapon] = (1 + fighters.reduce((c, f) => getWeapon(f) === weapon ? ++c : c, 0) / fightersNo).toFixed(2);
  }
